      ...
      def self.generate(definition)
        ...
        Class.new(self) do
          ...
          definition.fit_fields.each do |field|
            ...
            code << if field.scale && field.scale != 1
                      "scale = #{field.scale.inspect}.0\n"
                    else
                      "scale = nil\n"
                    end

            code << if field.dyn_data
                      "dyn = #{field.dyn_data}\n"
                    else
                      "dyn = nil\n"
                    end
          ...
          def get_dyn_value(dyn_data, raw_value)
            ...
            dyn_data.each do |key, dyn|
              ...
              if respond_to?("raw_#{dyn[:ref_field_name]}") &&
                 dyn[:ref_field_values].include?(send("raw_#{dyn[:ref_field_name]}"))
